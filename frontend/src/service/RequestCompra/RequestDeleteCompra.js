...
const deleteCompra = async (id) => {
  try {
    const response = await fetch(
      `${config.routeBase}/api/compra/delete/${id}`,
      {
        method: "DELETE",
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Error al eliminar la compra: ${response.status}`);
    }
    const compraEliminado = await response.json();
    return compraEliminado;
  } catch (error) {
